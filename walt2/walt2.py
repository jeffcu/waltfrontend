...
def create_checkpoint():
    try:
        ...
        bio_prompt_content = ""
        try:
            with open('walt2/walt_prompts/bio_creator_prompt.txt', 'r', encoding='utf-8') as f:  # ADDED: Read bio_creator_prompt.txt
...
def walt_process_checkpoint():  # RENAME function as well
    ...
    try:
        try:
            with open('walt2/walt_prompts/bio_creator_prompt.txt', 'r', encoding='utf-8') as f:  # Read bio_creator_prompt.txt
                bio_prompt_content = f.read()
        except Exception as e:
            logging.error(f"Error reading bio_prompt.txt: {e}")
